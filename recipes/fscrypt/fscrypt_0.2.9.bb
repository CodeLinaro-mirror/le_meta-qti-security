...
SRC_URI_append += " ${@bb.utils.contains_any('GCCVERSION', ['9.3%', '14.2%'], 'file://0001-solve-aarch64-oe-linux-gcc-build-error.patch', '', d)}"
SRC_URI_append += " ${@bb.utils.contains_any('GCCVERSION', ['9.3%', '14.2%'], 'file://fscrypt', '', d)}"
...
do_patch() {
	...
	if ${@bb.utils.contains_any('GCCVERSION', ['9.3%', '14.2%'], 'true', 'false', d)}; then
...
do_install() {
	install -d ${D}${bindir}
	install -d ${D}${libdir}
	if ${@bb.utils.contains('GCCVERSION', ['9.3%', '14.2%'], 'true', 'false', d)}; then
